...
export default function CartDrawer({ isOpen, onClose }) {
    ...
    const testCheckout = async () => {
        try {
            ...
            const response = await fetch("https://us-central1-airbrushnink-9f735.cloudfunctions.net/createStripeCheckoutSessionLive", {
                method: "POST",
                headers: {
                ...
                {cartItems.map((item, index) => (
                    <div key={index} className="flex flex-col gap-2 mb-4 border-b pb-3">
                        <div className="flex items-center gap-4">
                            <img src={item.image} alt={item.title} className="w-16 h-16 object-cover rounded shadow" />
                                ...
                                {isEditing ? (
                                    <>
                                        <div className="flex items-center gap-2 mt-2">
                                            ...
                                            <button
                                                onClick={() => item.quantity > 1 && updateCartItem(index, { quantity: item.quantity - 1 })}
                                    ...
                                    <>
                                        <p className="text-sm text-white/80">Size: {item.size} — Qty: {item.quantity}</p>
